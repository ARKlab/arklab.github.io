...
**Package v1.2.0.** Enables shared-mailbox activation on supported Mac, Windows and web Outlook setups. Update the existing Microsoft 365 deployment to manifest **1.2.0.0** after publishing Pages. See [shared-mailbox setup and acceptance](docs/SHARED-MAILBOXES.md). The approved design, own-profile `User.Read` permission, desktop SMTP alias handling and personal-mailbox mobile implementation remain. Mobile and shared-mailbox device acceptance are tracked separately; Microsoft 365 administrators control employee assignment, which publishing this release does not change. Follow [Android setup and acceptance](docs/ANDROID-SETUP.md) for the mobile test.
