...
**Package v1.1.0.** Adds automatic mobile signatures and a connection/preview command accessible from received messages. Android device acceptance is pending. Enabling mobile requires updating the existing Microsoft 365 deployment to manifest **1.1.0.0**; publishing Pages alone does not enable it. The approved design, own-profile `User.Read` permission and verified SMTP alias handling remain. Microsoft 365 administrators control employee assignment separately; publishing a release does not change that list. Follow [Android setup and acceptance](docs/ANDROID-SETUP.md).
